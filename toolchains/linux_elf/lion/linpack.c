...
void linpack_main(void)
{
  ...
  /* while (1) */
    {
      /* _printf("Enter array size (q to quit) [200]:  "); */
      /* fgets(buf,79,stdin); */
      /* if (buf[0]=='q' || buf[0]=='Q') */
      /* 	break; */
      /* if (buf[0]=='\0' || buf[0]=='\n') */
      ...
      if (arsize<10)
	{
      ...
      _printf("Memory required:  %ldK.\n",(memreq+512L)>>10);
      malloc_arg=(size_t)memreq;
      if (malloc_arg!=(size_t)memreq || (mempool=malloc(malloc_arg))==NULL)
	{
	  _printf("Not enough memory available for given array size.\n\n");
	  /* continue; */
	  exit(1);
	}
      _printf("\n\nLINPACK benchmark, %s precision.\n",PREC);
      _printf("Machine precision:  %d digits.\n",BASE10DIG);
      _printf("Array size %d X %d.\n",arsize,arsize);
      _printf("Average rolled and unrolled performance:\n\n");
      _printf("    Reps Time(s) DGEFA   DGESL  OVERHEAD    KFLOPS\n");
      _printf("----------------------------------------------------\n");
      nreps=1;
...
static REAL linpack(long nreps,int arsize)
{
  ...
  if (toverhead<0.)
    toverhead=0.;
  _printf("%8ld %6.2f %6.2f%% %6.2f%% %6.2f%%  %9.3f\n",
